...
function avatarFileInfo(string $uploadDir, int $userId): ?array
{
    foreach (['jpg', 'png', 'gif'] as $ext) {
        $path = $uploadDir . '/' . $userId . '.' . $ext;
        if (is_file($path)) {
            return [$path, 'uploads/' . $userId . '.' . $ext . '?v=' . filemtime($path)];
        }
    }
...
function saveAvatar(array $file, string $uploadDir, int $userId): array
{
    ...
    return move_uploaded_file((string) $file['tmp_name'], $targetPath)
        ? [true, '头像保存成功，页面已按 120px 头像尺寸缩放显示。']
        : [false, '头像保存失败，请检查 uploads 目录权限。'];
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    [$ok, $result] = saveAvatar($_FILES['pic'] ?? [], $uploadDir, $info['id']);
    if ($ok) {
        $message = $result;
    } else {
        $error = $result;
...
$avatarInfo = avatarFileInfo($uploadDir, $info['id']);
$displayAvatar = $avatarInfo[1] ?? $defaultAvatar;
?>
<!doctype html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        .box { width: 360px; margin: 70px auto; padding: 24px 28px; background: #fff; border: 1px solid #d4dae0; border-radius: 8px; box-shadow: 0 8px 24px rgba(20, 32, 45, 0.08); }
...
<body>
    <main class="box">
        ...
        <div class="profile">
            <img class="avatar" src="<?= htmlspecialchars($displayAvatar, ENT_QUOTES, 'UTF-8') ?>" alt="当前头像">
            <div>
                <div class="name">用户名：<?= htmlspecialchars($info['name'], ENT_QUOTES, 'UTF-8') ?></div>
                <p>当前头像</p>
            </div>
        </div>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <input id="pic" name="pic" type="file" accept="image/jpeg,image/png,image/gif" required>
