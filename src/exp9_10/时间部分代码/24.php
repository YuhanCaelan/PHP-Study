...
$now = new DateTime();
$isValid = $target instanceof DateTime;
$seconds = $isValid ? $target->getTimestamp() - $now->getTimestamp() : 0;
$isPast = $seconds < 0;
$absSeconds = abs($seconds);
$days = intdiv($absSeconds, 86400);
$hours = intdiv($absSeconds % 86400, 3600);
$minutes = intdiv($absSeconds % 3600, 60);
$remainSeconds = $absSeconds % 60;
$inputValue = $isValid ? $target->format('Y-m-d\TH:i') : '2026-06-30T00:00';
?>
<!doctype html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        .countdown strong { font-size: 28px; color: #175cd3; }
        .error { color: #b42318; }
    </style>
</head>
<body>
    <main class="box">
        ...
        <form method="post">
            ...
            <div class="row">
                ...
                <input id="target" name="target" type="datetime-local" value="<?= htmlspecialchars($inputValue, ENT_QUOTES, 'UTF-8') ?>">
            </div>
        ...
        <div class="countdown">
            ...
            <?php elseif ($isPast): ?>
                <?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?> 已经过了
                <strong><?= $days ?></strong> 天 <?= $hours ?> 小时 <?= $minutes ?> 分 <?= $remainSeconds ?> 秒。
            <?php else: ?>
                距离 <?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?> 还有
                <strong><?= $days ?></strong> 天 <?= $hours ?> 小时 <?= $minutes ?> 分 <?= $remainSeconds ?> 秒。
            <?php endif; ?>
        </div>
    </main>
